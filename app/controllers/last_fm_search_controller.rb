...
class LastFmSearchController < ApplicationController
  ...
  def display
   ...
   end_snip = CGI.escape(params[:query].to_s) + limit_snip + "&api_key=949a97d4b7ab1cf6227269acd34f39ce&format=json"
 ...
 def add_album
   ...
   this_album = Album.find_or_create_by(artist_id: this_artist.id, name: album_name, image_url: album_image, image_url_large: album_image_large)
     ...
     tracks_URI =  "http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=949a97d4b7ab1cf6227269acd34f39ce&artist=" + CGI.escape(artist_name) + "&album=" + CGI.escape(album_name) + "&format=json"
